...
/**
 * Durable Redis Streams consumer. Each subclass picks a stream name + consumer
 * group; ensure-group runs at module init and a background loop XREADGROUP's
 * new messages plus periodically XAUTOCLAIM's stale pending ones from other
 * consumer instances that died mid-processing.
 */
@Injectable()
export abstract class StreamConsumer implements OnModuleInit, OnModuleDestroy {
  ...
  /**
   * Periodically claims messages that have been pending for any consumer in
   * the group longer than `idleMs`. This is how we recover from a dead
   * consumer — the message never gets ACK'd, eventually we claim it.
   */
