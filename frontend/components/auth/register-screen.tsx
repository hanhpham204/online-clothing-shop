    ...
    <label className="flex w-full flex-col gap-2">
      ...
      <span className="relative block h-12">
        {iconUrl ? (
          ...
          <span
            aria-hidden
            className="pointer-events-none absolute left-4 top-1/2 block -translate-y-1/2 text-base text-white/40"
          >
            👤
          </span>
        )}
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(data.message ?? "Register failed. Please check your information.");
      }
        ...
        <div className="w-full max-w-[448px]">
          <article className="rounded-md border border-white/10 bg-black/40 px-5 py-5 shadow-[0px_25px_50px_0px_rgba(0,0,0,0.25)] backdrop-blur-[2px]">
            <header className="mb-10 text-center">
              <h1 className="text-4xl font-light uppercase tracking-[7.2px]">CREATE ACCOUNT</h1>
