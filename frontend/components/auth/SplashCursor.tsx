...
class Program {
  ...
  constructor(
    ...
    getUniformsFn: (program: WebGLProgram) => Record<string, WebGLUniformLocation | null>
  ) {
    this.gl = gl;
...
class Material {
  private gl: WebGLRenderingContext | WebGL2RenderingContext;
  private createProgramFn: (v: WebGLShader | null, f: WebGLShader | null) => WebGLProgram | null;
  private getUniformsFn: (program: WebGLProgram) => Record<string, WebGLUniformLocation | null>;
  private compileShaderFn: (type: number, source: string, keywords?: string[] | null) => WebGLShader | null;
  vertexShader: WebGLShader | null;
  ...
  setKeywords(keywords: string[]) {
    ...
    if (program == null) {
      const fragmentShader = this.compileShaderFn(this.gl.FRAGMENT_SHADER, this.fragmentShaderSource, keywords);
      program = this.createProgramFn(this.vertexShader, fragmentShader);
      this.programs[hash] = program;
    }

    if (program === this.activeProgram) return;
    if (program) this.uniforms = this.getUniformsFn(program);
...
export default function SplashCursor({
  ...
  useEffect(() => {
    ...
    const gradientSubtractProgram = new Program(gl, baseVertexShader, gradientSubtractShader, createProgram, getUniforms);
    const displayMaterial = new Material(gl, baseVertexShader, displayShaderSource, createProgram, getUniforms, compileShader);
    ...
    function calcDeltaTime(lastUpdateTime: number) {
      const now = Date.now();
      return { dt: Math.min((now - lastUpdateTime) / 1000, 0.016666), now };
    }
    ...
    function updateFrame() {
      if (!running) return;
      const delta = calcDeltaTime(lastUpdateTime);
      lastUpdateTime = delta.now;

      if (resizeCanvas()) initFramebuffers();

      colorUpdateTimer += delta.dt * config.COLOR_UPDATE_SPEED;
      if (colorUpdateTimer >= 1) {
        colorUpdateTimer = wrap(colorUpdateTimer, 0, 1);
        pointers.forEach((pointer) => (pointer.color = generateColor()));
      }
